...
def check_metric(list_metric: List)->tuple:
    """Check that through the steps, runs, algo and scenarios, the data share the same list of metrics"""
    if list_metric==[]:
        return True, []
    identical= True
...
def check_runs(list_nb_runs:List)->tuple:
    """Check that through the algos, and tasks, the data share the same number of runs"""
    if list_nb_runs==[]:
        return True, []
...
def data_preprocessing(
    ...
    """Function to check the json data format and reformulate it, if there is an issue"""
    ...
    for env in raw_data.keys():
        ...
        for task in raw_data[env].keys():
            ...
            algorithms_used.append(sorted(list(processed_data[env][task.lower()].keys())))

            for algorithm in raw_data[env][task].keys():
                # Up-case the algorithm name
                if algorithm.upper()!=algorithm:
                    processed_data[env][task.lower()][algorithm.upper()]=copy.deepcopy(raw_data[env][task][algorithm])
                    del processed_data[env][task.lower()][algorithm]
                
                # Append the number of runs used across the different algos, and tasks
                runs_used.append(len(processed_data[env][task.lower()][algorithm.upper()].keys()))

                for run in raw_data[env][task][algorithm].keys():
                    # Append the number of steps used across the different runs, algos and tasks
                    steps_used.append(len(processed_data[env][task.lower()][algorithm.upper()][run].keys()))
                    ...
                    for step in raw_data[env][task][algorithm][run].keys():
                        for metric in raw_data[env][task][algorithm][run][step].keys():
                            ...
                            if metric.lower()!=metric:
                                processed_data[env][task.lower()][algorithm.upper()][run][step][metric.lower()]=copy.deepcopy(raw_data[env][task][algorithm][run][step][metric])
                                del processed_data[env][task.lower()][algorithm.upper()][run][step][metric]
                        
                        # Append the metrics names used across the different steps, runs, algos and tasks
                        metrics_used.append(sorted(list(processed_data[env][task.lower()][algorithm.upper()][run][step].keys())))

                        # Up-case absolute metric
                        if "absolute" in step.lower() and step!="ABSOLUTE_METRIC":
                            processed_data[env][task.lower()][algorithm.upper()][run]["ABSOLUTE_METRIC"]=copy.deepcopy(raw_data[env][task][algorithm][run][step])
                            del processed_data[env][task.lower()][algorithm.upper()][run][step]
            ...
            if not valid_steps:
                for algorithm in common_algos:
                    for run in processed_data[env][task.lower()][algorithm].keys():
                        steps=list(processed_data[env][task.lower()][algorithm][run].keys())
                        if len(steps)!=common_nb_steps:
                            steps.remove("ABSOLUTE_METRIC")
                            for step in steps[common_nb_steps:]:
                                del processed_data[env][task.lower()][algorithm][run][step]
            if not valid_metric:
                for algorithm in common_algos:
                    for run in processed_data[env][task.lower()][algorithm].keys():
                        for step in processed_data[env][task.lower()][algorithm][run].keys():
                            aux=copy.deepcopy(processed_data[env][task.lower()][algorithm][run][step])
                            for metric in processed_data[env][task.lower()][algorithm][run][step].keys():
                                if metric not in common_metrics:
                                    del aux[metric]
                            processed_data[env][task.lower()][algorithm][run][step]=copy.deepcopy(aux)
